...
export class SetMenusService {
  ...
  private endpoint: string = 'https://staging.yhangry.com/booking/test/set-menus'; // hard coded for this assignment
  ...
  async fetchAndIngestData(): Promise<void> {
    ...
    try {
      ...
      for (const setMenuData of data) {
        ...
        await this.insertSetMenuCuisines(queryRunner, { set_menu_id: savedSetMenu.id, cuisine_id: setMenuData.cuisines[0].id});
...
async getFilteredSetMenus(
  ...
  const cuisineCountsMap = new Map<
    string,
    { name: string; slug: string; number_of_orders: number; set_menus_count: number }
  >();

  rawResults.forEach((result) => {
    // Group cuisines into menus
    let existingMenu = setMenus.find((menu) => menu.id === result.set_menu_id);
  ...
  private async insertCuisines(queryRunner, cuisine: CuisineDto): Promise<void> {
    const newCuisine = queryRunner.manager.create(Cuisine, cuisine);
    await queryRunner.manager.save(Cuisine, newCuisine);
  }

  private async insertSetMenu(queryRunner, setMenuData: SetMenuDto): Promise<SetMenu> {
    const setMenu = queryRunner.manager.create(SetMenu, setMenuData);
    return queryRunner.manager.save(SetMenu, setMenu);
  }

  private async insertSetMenuCuisines(queryRunner, setMenuCuisineData: SetMenuCuisineDto): Promise<void> {  
    const setMenuCuisine = queryRunner.manager.create(SetMenuCuisine, setMenuCuisineData);
    await queryRunner.manager.save(SetMenuCuisine, setMenuCuisine);
  }

  private async insertMenuGroups(queryRunner, menuGroupData: MenuGroupDto): Promise<void> {
